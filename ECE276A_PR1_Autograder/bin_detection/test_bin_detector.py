...
if __name__ == '__main__':
  ...
  for filename in os.listdir(folder):
    ...
    if filename.endswith(".jpg"):
      ...
      # # show image with actual bounding box (comment this out for submission as autograder doesn't take plots)
      ...
      # # plot bounding boxes on image (comment this out for submission as autograder doesn't take plots)
      # print('actual box: ', true_boxes)
      # img_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
      # for i in range(len(estm_boxes)):
      #   img_bgr = cv2.rectangle(img_bgr, (estm_boxes[i][0], estm_boxes[i][1]), (estm_boxes[i][2], estm_boxes[i][3]), (0,255,0), 2)
